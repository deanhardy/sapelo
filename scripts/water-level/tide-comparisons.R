...
int.date1 <- as.Date('2022-11-04') 
int.date2 <- as.Date('2022-11-07') 
...
## set dates and sites
df3 <- filter(df2, site_new %in% c('ML', 'T4-01', 'T1-02') & date_time_gmt >= int.date1 & date_time_gmt <= int.date2)

TEXT = 8
plot <- 
  ggplot(df3)  + 
  geom_line(aes(date_time_gmt, water_level_navd88, color = site_new)) +
  scale_x_datetime(name = 'Date', date_breaks = '12 hours', date_minor_breaks = '1 hour', date_labels = '%m/%d %H:%M') + 
  scale_y_continuous(name = 'Water Level (m NAVD88)', breaks = seq(-1.5,1.5,0.25), limits = c(-1.5,1.5), expand = c(0,0)) +
...
tiff(paste0(datadir, 'figures/tide-comps.tiff'), unit = 'in', height = 4, width = 6.5, res = 300)
plot
dev.off()
