...
nerr_wx_ba <- read.csv(file.path(datadir, 'water-level/nerr-data/sapmlmet-data/260713-sapmlmet/SAPMLMET.csv'),
    header = TRUE, skip = 2, stringsAsFactors = FALSE) %>%
    slice(., 1:n()) %>%
  mutate(date_time_est = mdy_hm(DateTimeStamp),
         BP = as.numeric(BP),
         F_BP = (F_BP),
...
nerr_wx_ai <- read.csv(file.path(datadir, 'water-level/nerr-data/sapmlmet-data/260713-sapmlmet-allinclusive/SAPMLMET.csv'),
                       header = TRUE, skip = 2, stringsAsFactors = FALSE) %>%
  slice(., 1:n()) %>%
  mutate(date_time_est = mdy_hm(DateTimeStamp),
         BP = as.numeric(BP),
