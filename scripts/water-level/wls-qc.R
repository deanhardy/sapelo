...
wls.field <- read_excel('/Users/dhardy/Dropbox/Sapelo_NSF/water_level_survey/data/sapelo-water-level-survey.xlsx', 
  ...
  select(date, gmt, site, serial, activity, type)
...
df <- read.csv(paste(datadir, 'wls_data.csv'))[,-1] %>%
  ...
  mutate(date = as.Date(date_time_gmt, '%m/%d/%y', tz = 'GMT'),
         site_date = paste(site, as.Date(date_time_gmt, '%m/%d/%y', tz = 'GMT')))

## testing filter methods
sites_list <- unique(df$site)
dates_list <- unique(field.smry$date)
df.test <- df %>% filter(site == sites_list[2] & between(date, as.Date(dates_list[60]-1), as.Date(dates_list[60]+1)))
  ...
  for (i in seq_along(sites_list)) {
    ...
    for (z in seq_along(gmt_list)) {
      ...
      OUTpost <- filter(df, site == sites_list[i] & between(df$date_time_gmt, gmt_list[z], gmt_list[z] + 1440)) %>%
                          mutate(field = 'post_dl')
      OUT <- rbind(OUTpre, OUTpost) %>%
        select(site, date, date_time_gmt, sensor_depth, logger, serial, field)
      qc <- rbind(OUT, qc)
...
qc.diff_post <- qc %>%
  ...
  pivot_wider(names_from = field, values_from = sensor_depth, values_fn = list) %>%
  ungroup() %>%
  hoist(pre_dl, previsit = list(1L)) %>%
  hoist(post_dl, postvisit = list(1L)) %>%
  select(!c(post_dl, pre_dl)) %>%
  ...
  mutate(post_pre = postvisit - previsit,
         ...
         logger_material = if_else(str_starts(serial, '219'), 'hobo_metal',
                         if_else(str_starts(serial, '1'), 'hobo_poly', 've_ceramic')),
         accuracy_x2 = if_else(logger_material == 'hobo_metal' & abs_diff > 0.003*2, 'outside',
...
qc.diff_post.fig<- 
  ...
  scale_y_continuous(name = 'Sensor Depth Difference (cm)', limits = c(0,90), breaks = seq(0,90,10)) + 
  ...
  scale_shape_manual(name = 'Logger',
                     ...
                     labels = c('Hobo U20L-04 (Poly)',
                                ...
                                'Van Essen Ceramic')) + 
  ggtitle(paste0('Quality Control: Absolute Difference in Sensor Depth Pre & Post Data Download')) + 
  facet_wrap('site')
qc.diff_post.fig
...
qc.graph <- function(df, na.rm = TRUE, ...){
  ...
  for (i in seq_along(sites_dates)) {
    ...
    for (z in seq_along(dates_list)) {
      ...
      plot <- 
        ggplot(df3)  + 
        geom_line(aes(date_time_gmt, sensor_depth)) +  
        ...
        scale_y_continuous(name = 'Sensor Depth (m)', 
                           breaks = seq(0,2,0.1), limits = c(0,2), expand = c(0,0)) +
