...
sap <- st_as_sf(df, coords = c('x', 'y'), crs = 4326) %>%
  mutate(year = 'NA', bill = 'NA', name = 'NA', parcel.id = 'NA', address = 'sapelo', due.date = 'NA',
         prior.payment = 'NA', amount.due = 'NA', paid.date = 'NA', owner = 'NA', 
         category = 'NA') %>%
  relocate(geometry, .after = category)
...
geo00 <- filter(geo2, year == 2000)
r00_lines <- points_to_lines(geo00, ids = 'address', names = 'category', order_matters = F) %>%
  filter(start == 'sapelo') %>%
  mutate(year = 2000)
...
for (z in seq_along(YR)) {
  ...
  fig <- ggplot() +
    ...
    geom_point(
      aes(color = category, geometry = geometry),
